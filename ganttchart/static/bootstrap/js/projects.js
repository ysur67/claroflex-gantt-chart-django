...
function add_project() {
    ...
    const projectsUrl = $('#add_project_btn').data('projects-url');
    $.post(projectsUrl,
        {
            mode: 'add_project',
            name: project_name,
        ...
        function (data) {
            ...
            //     $('#success').html('<div class="success">El proyecto se ha agregado con éxito.</div>');
...
function project_errors(data) {
    ...
    if (data['error']['project_name']) {
        error_text += '<div>El nombre del proyecto no puede estar vacío.</div>';
    ...
    if (data['error']['date']) {
        $.each(data['error']['date'], function (i, j) {
            ...
            if (j['date_finish'] == 1) {
                ...
                error_text += '<div>Fecha de finalización de la tarea #<b>' + this_task_num + '</b> no especificado.</div>';
            }

            if (j['valid'] == 1) {
                $('#task_date_start_' + j['task_id']).addClass('light_error_input');
                $('#task_date_finish_' + j['task_id']).addClass('light_error_input');
                error_text += '<div>Fecha de inicio de la tarea #<b>' + this_task_num + '</b> no puede ser inferior a la fecha de finalización.</div>';
            }
    ...
    if (data['error']['pre_date']) {
        $.each(data['error']['pre_date'], function (i) {
            ...
            error_text += '<div>Fecha de inicio de la tarea #<b>' + this_task_num + '</b> debe ser mayor que la fecha de finalización de la tarea. #<b>' + pre_task_num + '</b>.</div>';
    ...
    $.post('/ajax/ajaxProjects.php',
        ...
        function (data) {
            ...
            if ($(data['error']).size()) {
                ...
                document.location.reload();
                /*
                $('#success').html('<div class="success">Los datos se guardaron con éxito.</div>');
                clear_block_by_settime('success');
...
function add_project_task(after_task_id) {
    ...
    $('#select_task_user_' + task_id + '[not_select!=1]').easycomplete(
        {
            str_word_select: 'Seleccionar empleado',
...
function delete_project(project_id) {
    ...
    $.post('/ajax/ajaxProjects.php',
        ...
        function (data) {
            ...
            if (data == 1) {
                $('#project_' + project_id).replaceWith('<tr class="tb_data_1_row" id="project_' + project_id + '"><td colspan="5"><div class="success">Proyecto eliminado correctamente | <a href="javascript:;" onclick="restore_project(' + project_id + ');">Restablecer</a> | <a href="javascript:;" onclick="$(\'#project_' + project_id + '\').remove();">Esconder</a></div></td></tr>');
            }
...
// Acepte el Informe del Círculo de Obligaciones 
...
function recount_project_notice() {
    $.post('/ajax/ajaxProjects.php',
        ...
        function (data) {
            ...
            // Manejamos el contador en el menú de la izquierda
    ...
    $.post('/ajax/ajaxProjects.php',
        ...
        function (data) {
            ...
            if (data) {
                ...
                if (status == 'close') {
                    $('#success_close').html('<div class="success">Proyecto cerrado con éxito.</div>');
                } else {
                    $('#success_close').html('<div class="success">Proyecto abierto con éxito.</div>');
                }
...
function projects_tasks_init(static_task_obj) {
    // Si el objeto de tarea para el diagrama se generó en el lado del servidor
    if (!static_task_obj) {
        renumber_project_tasks();
...
function project_tasks_data_to_obj(e) {
    num = 1;
    tasks_obj = {};
    // Elegir la hora de inicio y finalización
    $('.task').each(function () {
...
function project_scheme_init(e, static_task_obj) {
    ...
    // Fecha de inicio de gráficos
    var min_date_start = null;
    // Fecha final de gráficos
    var max_date_finish = null;

    // Fecha de inicio de gráficos según plan
    ...
    // Primero, establecemos la fecha de los gráficos finales como la fecha del día actual
    max_date_finish = new Date().getTime();

    var tasks_count = 0;

    // Repasa las tareas y define los límites de los gráficos
    $.each(tasks_obj, function (i, task_data) {

        //var start_val = $(this).children('td').children('.date_start').val() || $(this).children('td').children('.date_start').text();
        //var finish_val = $(this).children('td').children('.date_finish').val() || $(this).children('td').children('.date_finish').text();

        // Objeto de tarea establecido estáticamente
        if (static_task_obj == 1 && !check_pr_task_for_visible_in_user_part_list(task_data)) {
            return true;
        }

        // Fechas de inicio y finalización de las tareas, teniendo en cuenta los turnos
    ...
    if (static_task_obj != 1) {
        // Define los límites del intervalo de tiempo para la tarea
    ...
    while (!stop_date) {
        ...
        // Contar cuántos días se mostrarán en un mes en un diagrama
    ...
    // Formando celdas para el gráfico
    var date_proc = date_start_timeseconds - 60 * 60 * 24 * 4;
...
function render_project_scheme_line(task_id, static_task_obj) {
    ...
    $.each(tasks_obj, function (i, task_data) {
        ...
        // Número de días en los que la tarea está retrasada
    ...
    $('.line[is_desc=1]').poshytip({
        content: function () {
            return $(this).children('.prline_desc_c').html()
        },
        className: 'tip-green',
        offsetX: -5,
        liveEvents: true,
...
function draw_line_dependence() {
    $('.line').each(function () {
        ...
        // Encontrar la gráfica de la tarea anterior
...
// Devuelve las fechas de inicio y finalización de las tareas
function get_task_dates(this_task_data) {
    ...
    if (Object.keys(prepends_tasks).length) {
        // El número de días en los que se cambia la tarea se basa en los anteriores
        task_offset_days = get_task_offset_days(prepends_tasks, this_task_data);

        // Designamos la fecha de inicio y finalización, teniendo en cuenta la compensación horaria
    ...
    // Fecha de finalización prevista de la tarea, teniendo en cuenta todos los turnos (si los hay)
    // La fecha de finalización se estima con respecto al turno y no refleja la fecha de finalización real
    ...
    // Si la tarea aún no se ha completado y la fecha de vencimiento ya ha expirado
    // Hacemos que la fecha de finalización sea REAL, es decir del día actual
    if (actual_mkdate > to_mktime(finish_date) && !isset_date(this_task_data['date_finished'])) {
        finish_date = date('Y-m-d');
    }
    // Si la fecha de finalización REAL es mayor que la fecha de finalización PLANIFICADA
...
// Obtiene el número de días en los que se cambia la tarea, si es necesario debido a tareas anteriores
function get_task_offset_days(prepends_tasks, this_task_data) {
    ...
    // Pasamos por todas las tareas anteriores
    $.each(prepends_tasks, function (i, j) {
        ...
        if (pre_iter_task_date_finish) {
            ...
            // Si hay días de compensación de tareas anteriores
        ...
        /* Averiguamos cuántos días está atrasada la tarea*/
        // Si la tarea aún no se ha completado y ya está vencida
...
// Obtiene una variedad de tareas anteriores
...
function show_project_period_date(min_date_start, max_date_finish, min_plan_date_start, max_plan_date_finish) {
    if ($.trim(min_date_start) == '' || $.trim(max_date_finish) == '') {
        ...
        var days_str = numToword(days, new Array('äåíü', 'äíÿ', 'äíåé'));
...
function show_project_tasks_comments(task_id, toclose) {
    // si la ventana de comentarios ya está abierta
...
// Aceptar El Círculo de Obligaciones Informe 
