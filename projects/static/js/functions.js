...
function sum_mask_proc(e) {
    var input_value, point_pos, str_before_point, str_after_point, input_range_start, input_range_end, range_obj,
        result_value;
...
function parseDate(input, format) {
    ...
    format.replace(/(yyyy|dd|mm|hh|ii|ss)/g, function (part) {
        fmt[part] = i++;
    });
...
function to_mktime(date, in_seconds) {  //if(g)alert(date+' '+parseDate(date, '', g))
    var dateobj = new Date(parseDate(date));
